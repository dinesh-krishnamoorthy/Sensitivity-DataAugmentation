...
for AugmentedFeedback = 0 % select whether to use data augmentation
 nr = 0; % counter to collect augmented data
nr2 = 0;% counter to collect expert data
% counter for simulator   
    for rollout = 1:25
        ...
        for run = 1:2
            if run == 1 % Current Policy Rollout
                for t = 1:par.nIter
                    ...
                    for sim_k = 1:20
                        ...
                        xf =  full(Fk.xf) ;
                        plant(rollout).x(:,ii) = xf;
                        plant(rollout).u(:,ii) = u_in;
                    end
                end
                if all(plant(rollout).x(1,200:end)<200*pi/180) && all(plant(rollout).x(1,200:end)>160*pi/180)
                    plant(rollout).flag = 1;
                else
                    plant(rollout).flag = 0;
                end
            end
            Policy(rollout) = NMPC;
            ...
            if run == 2 % Get expert feedback
                ...
                for t = 1:par.nIter
                    ...
                    if flag.success && AugmentedFeedback% Augment data using expert feedback
        ...
        if AugmentedFeedback
            ...
            Approx_policy = newgrnn(AugData.x,AugData.u,0.1);
        else
            % use only expert feedback to update policy
            Approx_policy = newgrnn(Expert.x,Expert.u,0.1);
        end
    ...
    if AugmentedFeedback
        save('plant_IE_Aug','plant')
        save('FullData_Aug','AugData')
        save('ExpData_Aug','Expert')
    else
        save('plant_IE_Exp','plant')
        save('ExpData_Exp','Expert')
    end
end
