...
clf
hold all
stairs(time,mpc.NMPC.x(1,:),'b','linewidth',2)
stairs(time,sparse.NMPC.x(1,:),'linewidth',2,'color',[1,1,1]*0.5)
stairs(time,full1.NMPC.x(1,:),'linewidth',2,'color',[0,0.5,0])
stairs(time,Aug.NMPC.x(1,:),'r','linewidth',2)
ylabel('$u [kW]$','Interpreter','latex')
legend('$\pi_{mpc}(x)$','$\pi_{approx}(x,\theta_0)$','$\pi_{approx}(x,\theta_1)$','$\pi_{approx}(x,\theta_2)$',...
    'Interpreter','latex')
